...
which can be either of B<SSL_CT_VALIDATION_PERMISSIVE>,
B<SSL_CT_VALIDATION_STRICT> or B<SSL_CT_VALIDATION_PUBLICTLS> as described below.
...
If validation mode is B<SSL_CT_VALIDATION_PUBLICTLS>, SCTs are only checked if
peer certificate have CA/B Forum Reserved Certificate Policy Identifiers. This
mode require at least 2 different valid logs, 3 if it's valid for longer than
180 days, as per Google and Apple CT policy.
